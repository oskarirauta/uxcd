...
- **Import from `docker run` / `docker-compose.yml` / stock uxc** `[L]` — `uxc import` / `uxc compose` translate Docker definitions into registry entries; `--dry-run` prints reviewable JSON. **Compose import** ✅ **DONE** — `uxc compose <file> [--dry-run]`: a small block-YAML reader translates a `docker-compose.yml` into registry entries sharing one infra netns (volumes / env / devices / depends_on / caps / restart; image or build via the linked converter; ports reported, not published). **docker run import** ✅ **DONE** — `uxc import <docker run ...> [--dry-run]`: parses a `docker run` line from raw argv (a leading `docker run` is tolerated) — `--name` (else derived from the image), `-v/--volume` (binds resolve against cwd; named -> `/srv/<name>/<vol>`), `--device`, `-e/--env`, `--cap-add/--cap-drop`, `--restart`, `--network host` and uxcd's `--infra`; ports + the trailing command + `--mount`/`--env-file`/`-w`/`-u`/etc. are warned, not applied. **stock-uxc adopt** ✅ **DONE** — `uxc import uxc <file.json> [name] [--dry-run]`: stock uxc already used `/etc/uxc/` and the `name`/`path`/`autostart` keys, so this just normalises the few that differ — `volumes` (stock's required-mounts list) -> `mounts`, `temp-overlay-size`/`write-overlay-path` -> `temp_overlay_size`/`write_overlay_path`; `jail`/`pidfile` dropped (warned). Nothing pulled (the bundle exists) or started; re-running preserves uxcd-only fields. **Import family complete** (docker run + compose + stock uxc), all one-shot **imports, not a runtime**.
...
- **In-container init (`cntrinit`)** `[M]` _(optional package)_ ✅ **PACKAGED** — a tiny, fully-static PID-1 (signalfd-based signal forwarding + zombie/orphan reaping + exit-status propagation; also a childless infra/pause init) for images that lack one. The maintainer's `cntrinit` (https://github.com/oskarirauta/cntrinit) was reviewed (adversarial multi-lens pass: core init path correct), fixed to **v1.2.0**, added as a uxcd submodule, and ships as the optional `cntrinit` opkg (static C, no deps). **Auto-injection decided against (manual only):** wiring it in as a bundle's init from docker2uxc/uxcd (a `none`/`bind`/`copy` opt-in) was scoped but dropped — uxcd's `cgroup.kill` already reaps on stop and healthchecks run externally, the infra is a netifd netns (no pause process), and most images either ship an init or are PID-1-aware, so auto-inject is niche and can even break s6/PID-1-assuming images. The optional `cntrinit` package being available for **manual** injection is enough.
